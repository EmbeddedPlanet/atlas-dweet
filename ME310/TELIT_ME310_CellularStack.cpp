...
TELIT_ME310_CellularStack::TELIT_ME310_CellularStack(ATHandler &atHandler, int cid, nsapi_ip_stack_t stack_type) : AT_CellularStack(atHandler, cid, stack_type)
...
{
#ifdef MBED_CONF_CELLULAR_OFFLOAD_DNS_QUERIES
    // _at.set_urc_handler("#QDNS:", mbed::Callback<void()>(this, &TELIT_ME310_CellularStack::urc_qdns));
#endif
...
bool TELIT_ME310_CellularStack::read_qdns(SocketAddress &address, nsapi_version_t _dns_version)
{
    _at.resp_start("#QDNS:");
...
void TELIT_ME310_CellularStack::urc_qdns()
{
    if (!_dns_callback) {
        return;
    }
...
#ifdef MBED_CONF_CELLULAR_OFFLOAD_DNS_QUERIES
nsapi_error_t TELIT_ME310_CellularStack::gethostbyname(const char *host, SocketAddress *address,
                                                        nsapi_version_t version, const char *interface_name)
{
    ...
    if (!address->set_ip_address(host)) {
        _at.set_at_timeout(60 * 1000);
        _at.cmd_start_stop("#QDNS", "=", "%s", host);

        _at.resp_start("#QDNS:");
        _at.restore_at_timeout();

        // Skip the echoed back hostname
        _at.skip_param();
        ...
        _at.read_string(ipAddress, sizeof(ipAddress));
        if (address->set_ip_address(ipAddress)) {
            if (version == NSAPI_UNSPEC || version == address->get_ip_version()) {
                success = true;
            }
        }
...
nsapi_value_or_error_t TELIT_ME310_CellularStack::gethostbyname_async(const char *host, hostbyname_cb_t callback,
                                                                       nsapi_version_t version, const char *interface_name)
{
    (void) interface_name;
    MBED_ASSERT(host);
...
nsapi_error_t TELIT_ME310_CellularStack::gethostbyname_async_cancel(int id)
{
    _at.lock();
    _dns_callback = NULL;
    _at.unlock();
